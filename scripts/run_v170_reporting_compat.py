"""Run V17 while tolerating only the stale inherited V13 final-print key.

The V13 harness writes all fold artifacts before its final console summary, but
that summary still requests poly_exact_accuracy. Current cardinality reports use
poly_cluster_accuracy/poly_accuracy. This wrapper recovers only that exact
post-write KeyError; every other exception still fails the job.
"""
...
from scripts import train_v170_permutation_set_matching as v170
...
def main(argv=None):
    args = v170.parser().parse_args(argv)
    v130.train_fold = _v130_train_fold_compat
    try:
        return v170.train_fold(args)
    finally:
