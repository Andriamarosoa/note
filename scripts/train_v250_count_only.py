"""V25 paired count-only softmax/conditional-ordinal experiment.
...
ARMS = ('categorical', 'conditional_ordinal')
SEED = 16061
MAX_EPOCHS = 20
...
def ordinal_probabilities(tf, logits):
    survival = tf.math.cumprod(tf.math.sigmoid(logits), axis=-1)
...
def build_model(arm, seed):
    ...
    if arm == 'categorical':
        ...
        logits = tf.keras.layers.Dense(6, name='v250_ordinal_logits')(hidden)
        out = tf.keras.layers.Lambda(lambda z: ordinal_probabilities(tf, z), name='v250_cardinality')(logits)
    model = tf.keras.Model(scaffold.inputs, out, name='v250_' + arm)
    # Pruning removes event-set, ranking and auxiliary losses from both arms.
...
def train(args):
    ...
    fit_table, final_table = class_weights(k[fit]), class_weights(k[final])
    ...
    protocol = {'experiment':'v250_count_only_ordinal_ab','only_arm_treatment':'7-way softmax versus 6-stage conditional ordinal P(K)',
                ...
                'max_epochs':MAX_EPOCHS,'epoch_selection':'inner weighted NLL, patience 4; refit from scratch',
                'fit_class_weights':fit_table.tolist(),'final_class_weights':final_table.tolist()}
...
def summarize(args):
    ...
    for fold in range(5):
        ...
        if set(r['arms']) != set(ARMS) or r['fold']!=fold: raise RuntimeError('incomplete fold')
