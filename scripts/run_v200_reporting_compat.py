"""Run V20 while tolerating only the inherited stale V13 final-print key."""
...
def _poly(card):
    for key in POLY_KEYS:
        if key in card:return float(card[key])
    raise RuntimeError(f"no supported poly accuracy key in {sorted(card)}")

def _compat(args):
    try:return _original(args)
    except KeyError as exc:
        if exc.args!=(LEGACY_KEY,):raise
        path=args.output_dir/f"report-fold-{args.outer_fold}.json"
        if not path.exists():raise RuntimeError(f"completed fold report missing: {path}")
        report=json.loads(path.read_text())
        if "v130" not in report.get("strata",{}).get("aggregate",{}):raise RuntimeError("written report missing pre-postprocess v130 key")
...
def main(argv=None):
    args=v200.parser().parse_args(argv); v130.train_fold=_compat
    try:return v200.train_fold(args)
    finally:v130.train_fold=_original
